...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):  # Plots one bounding box on image frame
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4):
    """
    Removes detections with lower object confidence score than 'conf_thresh'
    ...
    for image_i, pred in enumerate(prediction):
        ...
        # v = ((pred[:, 4] > conf_thresh) & (class_prob > .4))  # TODO examine arbitrary 0.4 thres here
        v = pred[:, 4] > conf_thres
        v = v.nonzero().squeeze()
        if len(v.shape) == 0:
            v = v.unsqueeze(0)
